...
-(void) parseInfo: (NSDictionary *)json {
    [self.storageService performBlockAsynchronously:^{
        ...
        NSString *unicodeCountry = [NSString stringWithCString:[country cStringUsingEncoding:NSNonLossyASCIIStringEncoding] encoding:NSUTF8StringEncoding];
        NSString *city = json[@"photo"][@"location"][@"locality"][@"_content"];
        NSString *unicodeCity = [NSString stringWithCString:[city cStringUsingEncoding:NSNonLossyASCIIStringEncoding] encoding:NSUTF8StringEncoding];
        NSString *location;
